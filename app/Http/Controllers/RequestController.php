...
class RequestController extends Controller
{
    ...
    public function algorithmv2(Request $request)
    {
...
//        dd($crawler->filterXPath('//*[@id="main-form"]/input')->extract(['value']));
        $csrf = $crawler->filterXPath('//*[@id="main-form"]/input')->extract(['value'])[0];
        $cookieJar = $this->cookieExtractor($res->cookies()->toArray());

        $resLogin = Http::asForm()->withHeaders([
            'Cookie' => $cookieJar
        ...
        try {
            $rJson = $this->dashboardExtractor($resLogin->body());
    ...
    function dashboardExtractor($html)
    {
//        $html = file_get_contents(asset('/html/file.html'));
        $crawler = new Crawler($html);
        $nama = $crawler->filter('#app > aside > div > div > div.info > a')->text();
        $no_pendaftaran = $crawler->filter('#app > aside > div > div > div.info > span')->text();
        $status_akun = $crawler->filter("#app > aside > div > nav > div > h5 > div")->text();

        $dataArray = $crawler->filter('tr')->each(function (Crawler $nodes, $i){
            try {

//                var_dump("nomor $i ------------ <br>");
//                var_dump($nodes->children('th')->html()."<br>");
//                var_dump($nodes->children('td')->html()."<br>");
//                var_dump("------------ <br>");
                if(in_array($i, array(10, 11, 12, 13, 14, 15, 16, 18, 19)))
                {
                    return $nodes->children('td')->html();
                }
        ...
        $responseJson = ['data' => [
            ...
            "universitas" => $dataArray[10],
            "program_studi" => $dataArray[11],
            "NIM" => $dataArray[12],
            "tgl_ditetapkan" => $dataArray[13],
            "no_KIPK" => $dataArray[14],
            "rekening" => $dataArray[15],
            "status_pencairan" => $dataArray[16],
            "biaya_hidup" => $dataArray[18],
            "biaya_pendidikan" => $dataArray[19]
        ]];

        return $responseJson;
    }

    function cookieExtractor($cookies): string
    {
        $cookieJar = "";
